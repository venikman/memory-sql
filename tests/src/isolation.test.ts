 ...
 * this test walks every .ts source in the three workspaces and asserts that
 ...
 * included so the SPEC rule "fetch-fhir.ts must not import effect" is
 * executable rather than grep-enforced. */
const SCANNED_DIRS = ["packages/core/src", "examples/src", "tests/src", "scripts"] as const

/** The ONLY files allowed to import effect / @effect/*. */
const ALLOWED = new Set<string>([
  "packages/core/src/effect.ts", // the adapter (subpath export memory-sql/effect)
  "examples/src/04-effect-adapter.ts", // the example demoing the adapter
  "tests/src/effect-adapter.test.ts" // the adapter's own test
])
 ...
 */
const EFFECT_IMPORT_PATTERNS: readonly RegExp[] = [
  /\bfrom\s*["'](?:effect|@effect\/[^"']*)["']/,
  /\bimport\s*\(?\s*["'](?:effect|@effect\/[^"']*)["']/,
  /\brequire\s*\(\s*["'](?:effect|@effect\/[^"']*)["']\s*\)/
]

const importsEffect = (source: string): boolean =>
  EFFECT_IMPORT_PATTERNS.some((pattern) => pattern.test(source))

/** All .ts sources under a repo-relative dir (recursive), as repo-relative posix paths. */
...
const allFiles: string[] = SCANNED_DIRS.flatMap(tsFilesUnder)
...
describe("isolation: effect stays quarantined in the adapter", () => {
  it("scans a real, non-empty source tree (the gate cannot pass vacuously)", () => {
    ...
    expect(allFiles).toContain("packages/core/src/index.ts")
    expect(allFiles).toContain("packages/core/src/store.ts")
  ...
  it("detector sanity: the adapter itself is seen importing effect", () => {
    ...
    const adapterSource = readFileSync(path.join(repoRoot, "packages/core/src/effect.ts"), "utf8")
